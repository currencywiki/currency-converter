...
Description: Super light currency converter widget by Currency.Wiki that offers Easy-To-Select colors to match your website theme with a preview function and not to mention 7 languages to select from - Spanish, Italian, French, Dutch, Hindi, Ukrainian, Russian and more with updates.
...
function wiki_create_menu_page() {
 
    add_menu_page('Currency Tool', 'Currency Tool', 'administrator', 'currency-wiki', 'wiki_menu_page_display', '');
 
} // end wiki_create_menu_page
add_action('admin_menu', 'wiki_create_menu_page');
 
/**
 * Renders the basic display of the menu page for the theme.
 */
function wiki_menu_page_display() {
    ob_start();
	...
	<div class="wrap">
    	...
    	<div class="col-md-6">
	    	<table>
				...
				<tr id="width-section" style="display: none;">
					<td>
						<label for="width" class="currency-wiki-widget-label wiki-width"><?php echo _e("Width"); ?>:</label>
					</td>
					<td>
						<input type="text" class="currency-wiki-widget-input" id="width" name="width" value="170" />
					</td>
				</tr>
				<tr id="height-section" style="display: none;">
					<td>
						<label for="height>" class="currency-wiki-widget-label wiki-height"><?php echo _e("Height"); ?>:</label>
					</td>
					<td>
						<input type="text" class="currency-wiki-widget-input" id="height" name="height>" value="280" />
					</td>
				</tr>
				<tr>
					<td>
						<label for="font_color" class="currency-wiki-widget-label"><?php echo _e("Font Color"); ?>:</label>
					</td>
					<td>
						<input type="text" class="currency-wiki-widget-input wiki-font-color" id="font_color" name="font_color" value="#FFFFFF" />
					</td>
				</tr>
				<tr>
					<td>
						<label for="style" class="currency-wiki-widget-label"><?php echo _e("Style"); ?>:</label>
					</td>
					<td>
						<input type="text" class="currency-wiki-widget-input wiki-style" id="style" name="style" value="#D51D29" />
					</td>
				</tr>
				<tr>
					<td>
						<label for="from" class="currency-wiki-widget-label"><?php echo _e("From"); ?>:</label>
					</td>
					<td>
						<select name="from" class="currency-wiki-widget-input wiki-from" id="from">
				...
				<tr>
					<td>
						<label for="to" class="currency-wiki-widget-label"><?php echo _e("To"); ?>:</label>
					</td>
					<td>
						<select name="to" class="currency-wiki-widget-input wiki-to" id="to">
							<?php foreach ($country_arr as $key => $value) : ?>
								<option value="<?php echo $key; ?>" 
				...
				<tr>
					<td>
						<label for="lang" class="currency-wiki-widget-label"><?php echo _e("Language"); ?>:</label>
					</td>
					<td>
						<select name="lang" class="currency-wiki-widget-input wiki-lang" id="lang">
				...
				<tr>
					<td colspan="2" style="text-align: right;">
						<button class="currency-wiki-widget-input-preview button button-wiki-widget">
							<?php _e("Preview"); ?>
						</button>
					</td>
				</tr>
		...
		<div class="col-md-6">
			<div id="currency-wiki-<?php echo $uniq_id; ?>">
				<a href="https://www.currency.wiki/"></a>
			</div>
		</div>
		<div class="col-md-12">
			<input type="text" id="shortcode-input" value='[currency_wiki w="170" h="280" s="D51D29" fc="FFFFFF" f="USD" t="EUR" type="fix"]' readonly />
		</div>
	</div>
	<?php
	...
	echo ob_get_clean();
} // end wiki_menu_page_display

function currency_wiki_shortcode( $atts ) {
	...
	ob_start();
	?>
	<div id="currency-wiki-<?php echo $uniq_id; ?>" class="currency-wiki">
		<a href="https://www.currency.wiki/"></a>
	</div>
	...
	<script type="text/javascript">
		...
		function widgetTrigger(ypFrame, type, lang) {
			...
			document.getElementById("currency-wiki-"+uniqID).appendChild(ypFrame);
	...
	<style>
        .currency-wiki iframe {border:none; outline: none;}
    </style>
	<?php
...
add_shortcode( 'currency_wiki', 'currency_wiki_shortcode' );
