<?php
class wiki_currency_widget_class extends WP_Widget {
	/**
	* Initialize the wiki widget
	**/
	public function __construct() {
		// This is where we add the style and script
        add_action( 'load-widgets.php', array(&$this, 'wiki_load_color_picker') );
		$widget_options = array( 
			'classname' => 'wiki_currency_widget',
			'description' => 'Display currency converter on your sidebar.',
		);
		parent::__construct( 'wiki_currency_widget', 'Currency Converter Widget', $widget_options );
	}

	/**
	* wiki widget load scripts
	* Method @wiki_load_color_picker
	*/
    function wiki_load_color_picker() {    
	...
	*/
	public function form( $instance ) {
		...
		<table>
			<tr>
				<td>
					<label for="<?php echo $this->get_field_id( 'title' ); ?>" class="currency-wiki-widget-label"><?php echo _e("Title"); ?>:</label>
				</td>
				<td>
			...
			<tr>
				<td>
					<label for="<?php echo $this->get_field_id( 'size' ); ?>" class="currency-wiki-widget-label"><?php echo _e("Type"); ?>:</label>
				</td>
    			<td>
			...
			<tr id="width-section-<?php echo $this->get_field_id( 'width' ); ?>" class="width-section-<?php echo $uniq_id; ?>" <?php echo ($size != 'custom') ? "style='display: none;'" : "" ; ?>>
				<td>
					<label for="<?php echo $this->get_field_id( 'width' ); ?>" class="currency-wiki-widget-label"><?php echo _e("Width"); ?>:</label>
				</td>
				<td>
			...
			<tr id="height-section-<?php echo $this->get_field_id( 'height' ); ?>" class="height-section-<?php echo $uniq_id; ?>" <?php echo ($size != 'custom') ? "style='display: none;'" : "" ; ?>>
				<td>
					<label for="<?php echo $this->get_field_id( 'height' ); ?>" class="currency-wiki-widget-label"><?php echo _e("Height"); ?>:</label>
				</td>
				<td>
			...
			<tr>
				<td>
					<label for="<?php echo $this->get_field_id( 'font_color' ); ?>" class="currency-wiki-widget-label"><?php echo _e("Font Color"); ?>:</label>
				</td>
				<td>
			...
			<tr>
				<td>
					<label for="<?php echo $this->get_field_id( 'style' ); ?>" class="currency-wiki-widget-label"><?php echo _e("Style"); ?>:</label>
				</td>
				<td>
			...
			<tr>
				<td>
					<label for="<?php echo $this->get_field_id( 'from' ); ?>" class="currency-wiki-widget-label"><?php echo _e("From"); ?>:</label>
				</td>
				<td>
			...
			<tr>
				<td>
					<label for="<?php echo $this->get_field_id( 'to' ); ?>" class="currency-wiki-widget-label"><?php echo _e("To"); ?>:</label>
				</td>
				<td>
				...
				<tr>
					<td>
						<label for="<?php echo $this->get_field_id( 'lang' ); ?>" class="currency-wiki-widget-label"><?php echo _e("Language"); ?>:</label>
					</td>
					<td>
						<select id="<?php echo $this->get_field_id( 'lang' ); ?>" name="<?php echo $this->get_field_name( 'lang' ); ?>" class="currency-wiki-widget-input lang" style="max-width: 160px; min-width: 160px;">
		...
		<?php if ( is_customize_preview() ) : ?>
			<script type="text/javascript">
				...
				jQuery(document).ready(function($) {
					...
					if (jQuery('.style-<?php echo $uniq_id; ?>').attr('id').replace('widget-wiki_currency_widget-', "").replace('-style',"") != '__i__') {
						jQuery(s).wpColorPicker({
			                change: _.throttle( function () { $(this).trigger('change'); }, 1000, {leading: false} )
			            });
					}

					if (jQuery('.font_color-<?php echo $uniq_id; ?>').attr('id').replace('widget-wiki_currency_widget-', "").replace('-font_color',"") != '__i__') {
...
function register_wiki_currency_widget() { 
	register_widget( 'wiki_currency_widget_class' );
}
add_action( 'widgets_init', 'register_wiki_currency_widget' );